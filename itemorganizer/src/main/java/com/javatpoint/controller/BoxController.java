...
//creating post mapping that post the student detail in the database
	@PostMapping("/box")
	private Integer saveBox(@RequestBody Box box, @Param(value = "shelfId") int shelfId) {

		return service.saveOrUpdate(box, 1);
	}

	@PostMapping("/addbox/{shelfId}")
	public Integer addBox(@PathVariable(value = "shelfId") int shelfId, @RequestBody Box box) {
		return service.saveOrUpdate(box, shelfId);
	}

	@PostMapping("/addItem/{shelfId}/{boxId}")
	public void addItem(@PathVariable(value = "shelfId") int shelfId, @PathVariable(value = "boxId") int boxId,
